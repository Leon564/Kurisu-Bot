...
class Message implements MessageBody {
  ...
  private constructor({
    ...
    this.userNumber = this.getUserNumber(data);
  ...
  private getUserNumber(data: proto.IWebMessageInfo): string {
    const [userId] = data.key.remoteJid!.split("@");
    if (isJidGroup(data.key.remoteJid!)) {
      const [participant] = data.key.participant!.split("@");
      return participant;
    }
    return userId;
  }
