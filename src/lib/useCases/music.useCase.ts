...
const music = async ({ data, utils }: UseCaseParams): Promise<SendData> => {
  ...
  try {
    ...
    const song = await mediaTube.get({ query: data.message.outCommandMessage! }).search()
    const tempFilename = `${Date.now()}-${song.title}.mp3`
    mp3Downloader.download(song.id, tempFilename)
    console.log('userId', data.userId)
    return new Promise((resolve, reject) => {
      mp3Downloader.on('progress', function (progress) {
      })
      mp3Downloader.on('finished', async (err, result) => {
        ...
        if (result.file.includes(tempFilename)) {
          ...
          utils.sendReply({type: 'audio',userId: data.userId , media: buffer, fakeQuoted: song.title, quoted: true, reacttion: '🎵', ptt:false, socket:data.socket})
        }
      })
    })
